#!/usr/bin/env bash
# Указать владельца бота: кому приходят отчёты о расходах и предупреждения
# о сбоях.
#
...
#
# Платный доступ эта команда не включает — он отдельно: set-paywall.sh
#
set -euo pipefail
...
if [ -z "$IDS" ]; then
  if [ -n "$current" ]; then
    ...
    echo "Владелец не задан — отчёты о расходах и предупреждения о сбоях"
    echo "отправлять некому."
  fi
  list_people
  echo
...
echo "Отчёты и предупреждения теперь приходят сюда."
echo "Платный доступ это не включает — он остаётся как был (bash set-paywall.sh)."
