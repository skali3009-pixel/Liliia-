...
umask 077
# Владелец: ему приходят отчёты о расходах и предупреждения о сбоях.
# Спрашиваем сразу — потом об этом вспоминают, только когда что-то уже
# случилось. Оплату это не включает: бот ставится бесплатным для всех.
ADMIN_IDS="$(read_env_value ADMIN_IDS)"
if [ -z "$ADMIN_IDS" ]; then
    echo
    echo "  Твой номер в Телеграме (только цифры). Он нужен, чтобы тебе"
    echo "  приходили отчёты о расходах и предупреждения о сбоях."
    echo "  Платный доступ он НЕ включает: бот ставится бесплатным для всех,"
    echo "  включить оплату можно потом — bash set-paywall.sh on"
