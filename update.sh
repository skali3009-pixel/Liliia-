...
if systemctl is-active --quiet "$SERVICE"; then
  ...
  # Заодно доставляем то, что владелец мог не запустить руками: сторожа,
  # ночные копии, автообновление. Отдельным процессом и после проверки, что
  # бот поднялся: что бы там ни случилось, обновление это не портит.
  if [ -f "$APP_DIR/ensure-ops.sh" ]; then
    bash "$APP_DIR/ensure-ops.sh" || log "Досоздание служб не удалось — обновление в порядке"
  fi
  exit 0
fi
