...
def _blind(text: str, place: str) -> bool:
    """Ошибка, о которой браузер не сказал ничего, кроме самого факта.

    `window.onerror` отдаёт «Script error.» без файла и строки ровно в одном
    случае: поломка случилась **внутри чужого скрипта**, загруженного с
    другого адреса. Браузер прячет подробности намеренно — иначе чужая
    страница могла бы читать чужой код по тексту ошибок.

    В приложении такой скрипт ровно один — `telegram.org/js/telegram-web-app.js`,
    сам Telegram. Наш `app.js` лежит на своём адресе, и его ошибки приходят с
    текстом, файлом и строкой; значит, «Script error.» без файла — это не наш
    код, и починить его нам нечем.

    Поймано у Лилии на живом телефоне: пришло «🐞 Приложение сломалось» с
    единственной строкой «Что: Script error.» — без места, без подробностей и
    без возможности что-то сделать. А сообщение при этом утверждало, что
    человек видел пустой или застывший экран, хотя знать этого мы не можем.
    Тревога, на которую нельзя ответить, учит не читать тревоги вообще.

    Вылечить это по-настоящему можно было бы `crossorigin="anonymous"` на
    теге скрипта — тогда браузер показал бы подробности. Но если telegram.org
    не пришлёт нужный заголовок, браузер откажется выполнять скрипт совсем, и
    приложение не откроется НИ У КОГО. Проверить это отсюда нельзя: прокси
    сессии не пускает к telegram.org (403). Непроверенная правка такой цены
    не стоит.
    """
    return not place and text.rstrip(".").strip().lower() == "script error"
...
async def report_client(bot: Bot | None, message: str, *, where: str,
    ...
    if _blind(text, place):
        # Браузер спрятал подробности, и сказать владелице нечего. Молчим —
        # но в журнале запись выше осталась.
        return False
