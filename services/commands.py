...
#
# Команды /referral здесь нет намеренно, хотя она работает. Список держится
# на десяти строках, и одиннадцатая вытеснила бы не «лишнюю» — вытеснять
# нечего: остальное либо ежедневное, либо обязано быть на виду по закону.
# А пока доступ бесплатен, «Позвать подругу» в меню обещает подарок, которым
# нельзя воспользоваться. Место для неё выбирает Лилия в тот день, когда
# включается оплата.
...
ADMIN: tuple[tuple[str, str], ...] = (
    ...
    ("status", "Что с ботом: версия, диск, картинки"),
    ("report", "Отчёт сейчас: день или неделя"),
    ("sources", "Откуда приходят и доходят ли"),
    ("backup", "Сделать копию базы прямо сейчас"),
    ("grant", "Выдать доступ человеку"),
    ("circles", "Показать кружки знакомства"),
