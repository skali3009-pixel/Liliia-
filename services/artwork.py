...
Картинки здесь не лежат в git — они большие и меняются отдельно от кода.
Бот при старте проверяет, что все на месте, и молча докачивает недостающие.
Если сеть недоступна, приложение просто покажет градиентное ночное небо:
падать из-за картинок бот не должен.

Шапка «Спорта» — исключение: она лежит прямо в репозитории (gym.webp,
33 КБ) и здесь не числится. Скачивание с CDN на сервере не сработало, и
вкладка открывалась чёрной полосой. Картинка, которая приезжает вместе с
кодом, не зависит ни от сети, ни от чужого хранилища.
"""
...
if __name__ == "__main__":  # ручной запуск: python -m services.artwork
    ...
    got = asyncio.run(ensure_artwork(force=True))
    left = missing()
    print()
    print(f"Скачано: {len(got)} из {len(ARTWORK)}")
    if left:
        # Раньше здесь была только цифра. Ноль без объяснения выглядит как
        # «ничего не делал», хотя на самом деле причина уже напечатана выше
        # — и человек, который её не ищет, остаётся без картинок и без
        # понимания почему.
        print(f"НЕ СКАЧАЛИСЬ: {', '.join(left)}")
        print("Причина — в строках выше: там написано, что ответила сеть.")
        print("Если там «Cannot connect» или «timeout» — сервер не достаёт")
        print("до хранилища картинок. Пришли эти строки, и я положу картинки")
        print("прямо в код, как уже сделано с шапкой «Спорта».")
    else:
        print("Все картинки на месте.")
