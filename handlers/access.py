...
async def owner_backup(message: Message) -> None:
    """Копия базы по требованию, без консоли хостинга.

    Копии и раньше делались сами каждую ночь, но убедиться в этом и
    сделать свежую можно было только из консоли сервера — а Лилия работает
    с айпада. Просьба была прямая: «сделай всё максимально сам». Команда и
    есть это «сам»: та же `backup.sh`, что работает по расписанию, просто
    запускается отсюда.

    Второй способ копировать заводить нельзя: он разошёлся бы с первым в
    тот же день — например, забыл бы фотографии, — и узнали бы об этом
    ровно тогда, когда копия понадобилась.
    """
    if message.from_user.id not in config.ADMIN_IDS:
        return   # для остальных команды словно не существует
    ...
    # Отвечаем сразу: копия базы с фотографиями идёт минутами, и молчание
    # всё это время человек читает как «команда не сработала».
    await message.answer("🗄 Делаю копию базы и фотографий.\n"
                         "Это займёт от полуминуты до нескольких минут — "
                         "готовый архив придёт сюда файлом.")

    # В поток, а не в общий цикл: внутри чужие процессы (pg_dump, tar), и в
    # цикле это остановка всего бота. Своего цикла событий здесь не заводим —
    # соединения с базой принадлежат тому, в котором живёт бот.
    получилось, хвост = await asyncio.to_thread(backups.сделать)

    if получилось:
        строки = await asyncio.to_thread(backups.строки)
        await message.answer("✅ Копия готова.\n\n" + "\n".join(строки))
        return

    await message.answer(
        "🛑 Копия не сделалась.\n\n"
        f"Что ответил сервер:\n{хвост}\n\n"
        "Данные при этом целы — не сделалась именно копия. "
