...
// `hold`, корректирующий пакет — `static_hold`. Сверяться с одним словом
// значит потерять свечение на следующем пакете, и молча.
function trainerIsHold(exerciseId) {
  return (trainerFor(exerciseId)?.animationType || '').includes('hold');
}
