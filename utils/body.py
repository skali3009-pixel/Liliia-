...
# Ниже этого пропорциональная модель не сжимает фигуру-ориентир. Предел
# нужен для странных целей, но слишком высокий предел врёт при честном
# большом похудении: со 120 до 70 кг талия уходит куда сильнее, чем на 22%.
GOAL_FLOOR = 0.62

# Диапазон веса считаем по ИМТ. Это грубый ориентир для роста, а не
# медицинская норма: он не знает ни про мышцы, ни про телосложение, ни про
# возраст. Поэтому и называем его ориентиром, а не «здоровым весом».
...
def healthy_weight_range(height_cm: float) -> tuple[int, int] | None:
    """Ориентировочный диапазон веса для роста — по ИМТ 18,5–24,9."""
    if height_cm <= 0:
        return None
...
def goal_silhouette(
    ...
    *,
    ...
    Если замеров нет и фигура «сейчас» сама прикинута по весу, то и цель
    честнее прикинуть по целевому весу — это просто тело такого веса и
    роста. Пропорциональная модель тут только испортила бы: она тянет за
    собой все погрешности прикидки и упирается в предел сжатия.

    Если замеры есть, форма тела своя, и её надо сохранить: тогда объём
    уводим пропорционально. Вес меняется вместе с объёмом, а объём при
    неизменном росте — как квадрат ширины, отсюда корень. Дальше изменение
    раскладывается по зонам: талия отзывается сильнее, руки и ноги слабее.
    """
    if weight_kg <= 0 or target_weight_kg <= 0 or target_weight_kg >= weight_kg:
        return None

    if estimated and height_cm > 0:
        figure, _ = build_silhouette(
            measures={}, height_cm=height_cm, weight_kg=target_weight_kg
        )
        return figure
    ...
    *,
    ...
    if height_cm:
        ...
        if healthy:
            low, high = healthy
            insights.append(
                Insight("📐", "Ориентир по весу",
                        f"{low}–{high} кг для роста {round(height_cm)} см. "
                        "Это грубая прикидка по росту — она не знает ни про "
                        "мышцы, ни про телосложение")
            )
    ...
    # Формулировки здесь намеренно осторожные. Приложение не ставит диагнозов
    # и не обещает физиологического результата: оно показывает ориентиры, а
    # что произойдёт у конкретного человека, зависит от него, а не от нас.
    if water_ml:
        # Запятая ставится только в числе. Раньше `.replace(".", ",")` шёл по
        # всей строке и съедал точку между предложениями: получалось
        # «2,1 л воды, Шаги и растяжка» — с большой буквы после запятой.
        litres = f"{water_ml / 1000:.1f}".replace(".", ",")
        insights.append(
            Insight("💧", "Вода и движение",
                    f"{litres} л воды в день. Шаги и растяжка тоже в счёт")
        )
    ...
    if protein_g:
        insights.append(
            Insight(
                "🏋️",
                "Белок",
                f"{protein_g} г белка в день. С силовыми 2–3 раза в неделю "
                "это может помочь сохранить мышцы",
            )
        )
...
def zones(measures: dict[str, float]) -> list[dict[str, Any]]:
    ...
    """
    # «Нога» вместо «бедро»: рядом с «бёдрами» второе слово не читается.
    titles = [
        ("arm", "Рука", "arm"),
        ("bust", "Грудь", "chest"),
        ("waist", "Талия", "waist"),
