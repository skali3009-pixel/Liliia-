...
class User(Base):
    ...
    # Личный ключ для автоматической присылки шагов с телефона. Не пароль:
    # по нему можно только записать себе шаги, прочитать нельзя ничего.
    # Меняется в один шаг — так отзывается ссылка, отправленная не туда.
