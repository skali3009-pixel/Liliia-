...
def test_счётчик_считает_по_настоящему_списку():
    ...
    assert O.ВСЕГО_ШАГОВ == len(O.ШАГИ) == 7
    assert [O.номер_шага(ш) for ш in O.ШАГИ] == list(range(1, 8))

    async def scenario():
        м = ФейковоеСообщение()
        await O.спросить(м, O.ШАГИ[2])
        текст = м.отправлено[0][0]
        assert текст.startswith(f"Вопрос 3 из {O.ВСЕГО_ШАГОВ}")
        assert O.ШАГИ[2].текст in текст
    run(scenario)


def test_в_анкете_только_то_без_чего_нет_нормы():
    """Каждый вопрос обязан быть нужен для нормы — или быть названным исключением.

    Анкету сократили с девяти вопросов до семи ровно по этому правилу: норма
    считается из пола, возраста, роста, веса, активности и цели, а целевой
    вес и аллергии в неё не входят вовсе — их спрашиваем после, когда норма
    уже на руках.

    Сторож смотрит на саму формулу, а не на список из головы: добавят в
    расчёт новое поле — тест потребует вопрос; добавят вопрос просто так —
    потребует объяснить, зачем он здесь.
    """
    import inspect

    from utils.formulas import calculate_macros, daily_water_ml

    нужно = set()
    for функция in (calculate_macros, daily_water_ml):
        нужно |= set(inspect.signature(функция).parameters)

    # Как поле расчёта зовётся вопросом анкеты.
    ИМЕНА = {
        "gender": "gender",
        "age_years": "age",
        "height_cm": "height",
    ...
    assert нужно <= set(ИМЕНА), f"в расчёте появилось поле без вопроса: {нужно - set(ИМЕНА)}"

    # Единственное исключение, и оно названо: тип питания в норму не входит,
    # но это один тап, и без него вегану в первый же день предложат мясо.
    ИСКЛЮЧЕНИЯ = {"diet_type"}

    спрашиваем = {ш.состояние.state.split(":")[1] for ш in O.ШАГИ}
    assert спрашиваем == {ИМЕНА[поле] for поле in нужно} | ИСКЛЮЧЕНИЯ, спрашиваем
...
def test_возврат_предлагает_именно_тот_вопрос():
    async def scenario():
        м = ФейковоеСообщение()
        шаг = O.ПО_СОСТОЯНИЮ[OnboardingStates.activity_level.state]
        await O.предложить_продолжить(м, шаг)

        текст, кнопки = м.отправлено[0]
        assert "5 из 7" in текст and "Осталось 3" in текст
...
def test_письмо_застрявшей_на_середине():
    async def scenario():
        async with db() as session:
            ...
            assert "4 из 7" in письма[0].text
    run(scenario)
...
def test_без_известного_шага_номер_не_выдумывается():
    ...
    assert f"{O.ВСЕГО_ШАГОВ} коротких вопросов" in текст
