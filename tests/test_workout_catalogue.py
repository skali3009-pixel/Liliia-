...
def test_the_catalogue_has_almost_no_duplicates():
    ...
    Список закрытый нарочно. Повтор — это не ошибка сама по себе, но
    появляться он должен осознанно: одно упражнение в двух программах
    показывает один ролик, а вот два разных движения под одним названием
    учили бы не тому. Шесть нижних появились с блоком 15: «Второй
    подбородок» собран из упражнений, которые уже сняты для «Холки»,
    «Фейс-йоги» и «Самомассажа».
    """
    ...
    assert set(repeated) <= {"Планка на локтях", "Супермен лёжа", "Кошка-корова",
                             ...
                             "«Стенка»: скольжение руками по стене",
                             "Втягивание подбородка у стены",
                             "Вытягивание шеи вверх",
                             "Выдвижение челюсти с запрокинутой головой",
                             "Массаж по линии нижней челюсти",
                             "Отток по шее вниз к ключицам"}, repeated
