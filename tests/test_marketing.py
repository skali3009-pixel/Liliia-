...
def test_карточка_говорит_то_что_просили():
    ...
    for кусок in ("Я Лилия, автор AURA и музыкального проекта SCALIA",
                  ...
                  "Если захочешь послушать — открой страницу проекта"):
        assert кусок in music.ТЕКСТ, кусок

    # Обращение — на «ты», как во всём остальном боте. Решение Лилии 22.09:
    # «вы» в одном сообщении из сорока читается как чужая вставка. Ловим
    # обе половины фразы: первая версия правки поменяла только «захотите»
    # и оставила «откройте» — то есть опечатку посреди одного предложения.
    for на_вы in ("захотите", "откройте", "послушаете", "ваш"):
        assert на_вы not in music.ТЕКСТ.lower(), на_вы
